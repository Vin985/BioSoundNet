...
class BioSoundNet(AudioDetector):
    """BioSoundNet Network with one less Dense layer to reduce the number of parameters and overfitting

    Args:
        AudioDetector ([type]): [description]
...
class BioSoundNetBinary(BioSoundNet):
    def get_top_layers(self, x=None):
        if x is None:
            x = self.get_base_layers()
        x = layers.BatchNormalization()(x)
